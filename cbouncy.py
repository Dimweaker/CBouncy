...
class CBouncy:
    ...
    async def run(self):
        ...
        for case in case_list:
            ...
            cp.generate(case, self.mutate_num)
...
async def run(tmp_path: str, epoch_i: int, generate_num: int = 5, mutate_num: int = 10,
    ...
    start = time.time()
    test_dir = tmp_path.join(f"/test_{epoch_i}")
